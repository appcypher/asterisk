...
use crate::models::{ModelResult, OpenAIResponseStreamError};
...
impl ResponseStream {
    ...
    fn map_error(err: Error) -> ModelResult<String> {
        let error = match err {
            Error::InvalidStatusCode(_, response) => {
                OpenAIResponseStreamError::OpenAIResponse(response)
            }
            err => OpenAIResponseStreamError::EventSourceError(err),
        };
